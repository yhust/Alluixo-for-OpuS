...
import alluxio.master.OpuS.OpuSMaster;
import alluxio.master.OpuS.User;
...
public final class DefaultFileSystemMaster extends AbstractMaster implements FileSystemMaster {
  ...
  @Override
  public synchronized String getOpuSInfo(AlluxioURI path, int userId, GetOpuSInfoTOptions options)
          throws FileDoesNotExistException, AccessControlException, InvalidPathException {
    // Metrics.GET_FILE_INFO_OPS.inc();
    LOG.info("Received request from Opus client " + userId + " for file " + path);
    FileInfo fileInfo = getFileInfo(path, new GetStatusOptions());
    // boolean isHit = fileInfo.getInMemoryPercentage() == 100;
    long fileSize = fileInfo.getLength();
    String opuSInfo = "";
    try{
      opuSInfo = OpuSMaster.access(userId, path, fileSize);
    }
    catch (IOException e)
    {
      LOG.info("ERROR while trying to write the access information");
    }
