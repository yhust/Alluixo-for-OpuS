...
public final class FileSystemMasterClientServiceHandler implements
  ...
  @Override
  public GetOpuSInfoTResponse getOpuSInfo(final String path, final int userId, final GetOpuSInfoTOptions options) throws AlluxioTException {
    return RpcUtils.callAndLog(LOG, new RpcCallable<GetOpuSInfoTResponse>() {
      @Override
      public GetOpuSInfoTResponse call() throws AlluxioException {
        return new GetOpuSInfoTResponse( //ThriftUtils.toThrift(
                mFileSystemMaster.getOpuSInfo(new AlluxioURI(path), userId, options));
      }

      @Override
      public String toString() {
        return String.format("GetOpuSInfo: path=%s, userId=%s", path, userId);
      }
    });
  }
